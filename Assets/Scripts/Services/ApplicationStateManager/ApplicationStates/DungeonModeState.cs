...
namespace Services
{
    public class DungeonModeState : BaseApplicationState
    {
        public readonly string UI_PREFAB = UIPrefabs.UIDungeonMode;

        private UIWidget _uiWidget;

        private int _levelIndex = (int)Constants.SceneIndexes.DUNGEON_SCENE;

        public DungeonModeState()
        {
        ...
        protected override void SetToActiveStateFromBackground(StackStateMachineBaseState prevState, Dictionary<string, object> options)
        {
            ...
            if (ServiceLocator.Instance.Get<LevelSceneManager>().GetLevelIndex() != _levelIndex)
            {
